...
int main(int nargs, char *argv[])
{
    ...
    if (N % nbp != 0)
    {
        std::cout << "Veuillez entrer un multiple du nombre de lignes : " << N;
        return 0;
    }
    ...
    MPI_Allreduce(buffer.data(), result.data(), N, MPI_DOUBLE, MPI_SUM, globComm);

    std::cout << "process " << rank << "received " << result << std::endl;
