...
auto sdl2::formated_text::font_manager_t::load_and_add(const std::string &key,
      ...
      qformat()
{
    ...
    formats[style::texte] = {this->font_manager["normal"], {0xA0, 0xA0, 0xA0, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 8};
    formats[style::italic] = {this->font_manager["italic"], {0xB0, 0xB0, 0xB0, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 8};
    formats[style::bold] = {this->font_manager["bold"], {0xFF, 0xFF, 0xFF, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 8};
    formats[style::italic_bold] = {this->font_manager["bolditalic"], {0xE0, 0xFF, 0xFF, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 8};
    formats[style::heading_1] = {this->font_manager["heading1"], {0xFF, 0xFF, 0x00, 0xFF}, {0x00, 0x00, 0xF0, 0xFF}, 0};
    formats[style::heading_2] = {this->font_manager["heading2"], {0x00, 0xFF, 0xFF, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 2};
    formats[style::heading_3] = {this->font_manager["heading3"], {0xFF, 0xFF, 0xFF, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 4};
    formats[style::heading_4] = {this->font_manager["heading4"], {0xFF, 0xFF, 0xFF, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 6};
    formats[style::heading_5] = {this->font_manager["heading5"], {0xFF, 0xA0, 0xA0, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 8};
    formats[style::heading_6] = {this->font_manager["heading6"], {0xFF, 0x00, 0x00, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 10};
    formats[style::code] = {this->font_manager["code"], {0xFF, 0xFF, 0xA0, 0xFF}, {0x00, 0x00, 0xA0, 0xFF}, 0};
    formats[style::link] = {this->font_manager["link"], {0xFF, 0xA0, 0xFF, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 0};
    this->qformat = {{0xFF, 0xFF, 0xFF, 0xFF}, {0x40, 0x40, 0x40, 0xFF}, 32};
    for (int i = 0; i < max_list_level; ++i)
    {
        this->ordered_list_formats[i] = {{0xFF, 0xFF, 0xFF, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 32 + 16 * i, numbering::latin};
        this->unordered_list_formats[i] = {{0xFF, 0xFF, 0xFF, 0xFF}, {0x00, 0x00, 0x00, 0xFF}, 32 + 16 * i, L'•'};
    }
}
// =================================================================================================
bool sdl2::formated_text::load_simplified_markdown(const std::string &path)
{
#if defined(DEBUG)
    std::cerr << __PRETTY_FUNCTION__ << std::endl
              << std::flush;
...
#if defined(DEBUG)
    std::cerr << "Fichier charge : " << std::endl
              << std::flush;
#endif
    while (fich)
    {
    ...
    fich.close();
#if defined(DEBUG)
    std::cerr << "Fin chargement ligne à ligne" << std::endl
              << std::flush;
#endif

    // On va adapter maintenant le texte à la zone dans laquelle on veut afficher et preparer les
    ...
    {
#if defined(DEBUG)
        std::cerr << "Traitement ligne " << line << std::endl
                  << std::flush;
#endif
        if (line.size() == 0)
        {
        ...
        {
#if defined(DEBUG)
            std::cerr << "Ligne de code : " << std::endl
                      << std::flush;
#endif
        ...
        if (current_style != style::texte)
        {
            ...
            current_style = style::texte;
#if defined(DEBUG)
            std::cerr << "Fin d'une commande uni ligne" << std::endl
                      << std::flush;
#endif
            continue;
        }
        ...
        {
#if defined(DEBUG)
            std::cerr << "C'est du texte..." << std::endl
                      << std::flush;
#endif
...
// =================================================================================================
void sdl2::formated_text::begin_at_line(window &w, int iline)
{
#if defined(DEBUG)
    std::cerr << __PRETTY_FUNCTION__ << std::endl
              << std::flush;
    ...
    SDL_FillRect(panneau, nullptr, SDL_MapRGB(panneau->format, this->formats[style::texte].papier.red, this->formats[style::texte].papier.green, this->formats[style::texte].papier.blue));
    style current_style = style::texte;
    SDL_Rect pos;
    pos.y = 0;
    ...
    {
#if defined(DEBUG)
        std::cerr << "Analyse ligne n°" << iwords << std::endl
                  << std::flush;
        ...
        if (this->splitted_string[iwords].size() == 1)
        {
            ...
            // Il faut d'abord gerer si ce n'est pas par hasard des barres horizontales à afficher !
            if (word == "****")
            {
#if defined(DEBUG)
                std::cerr << "Affichage ligne type 1 en " << pos.y << std::endl
                          << std::flush;
#endif
...
#if defined(DEBUG)
        std::cerr << "Pas une ligne. On analyse" << std::endl
                  << std::flush;
#endif
        ...
        for (std::size_t iword = 0; iword < words.size(); ++iword)
        {
            ...
            {
#if defined(DEBUG)
                std::cerr << "Preparation d'un titre" << std::endl
                          << std::flush;
#endif
                current_style = style::heading_1;
                beg_word = 2;
            ...
            std::string true_word(word.data() + beg_word, word.data() + end_word);
            if ((add_space) and (iword != words.size() - 1))
                true_word += " ";
#if defined(DEBUG)
            std::cerr << "On rajoute le mot : " << true_word << std::endl
                      << std::flush;
#endif
            int wt, ht;
            TTF_SizeText(this->formats[current_style].pt_fonte->m_pt_ttf, true_word.c_str(), &wt, &ht);
#if defined(DEBUG)
            std::cerr << "Taille du mot en pixel : " << wt << " x " << ht << std::endl
                      << std::flush;
#endif
...
#if defined(DEBUG)
            std::cerr << "Affichage en : " << pos.x << ", " << pos.y << std::endl
                      << std::flush;
            ...
            std::cerr << "Couleur du papier : " << int(this->formats[current_style].encre.red) << ", "
                      ...
                      << int(this->formats[current_style].encre.blue) << std::endl
                      << std::flush;
#endif
            ...
            SDL_FillRect(panneau, &papier_fill, SDL_MapRGB(panneau->format, this->formats[current_style].papier.red, this->formats[current_style].papier.green, this->formats[current_style].papier.blue));
    ...
    this->position_and_stretching.x = 0;
    this->position_and_stretching.y = 0;
