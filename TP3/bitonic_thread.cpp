...
namespace Bitonic
{
    ...
    sort(bool up, std::vector<Obj> &x)
    {
        ...
        // Je fais ici avec deux threads mais on peut généraliser avec un vecteur de threads et une boucle for.

        std::thread t1(&_sort<Obj>, up, x.data(), len / 2);
        std::thread t2(&_sort<Obj>, !up, x.data() + len / 2, len - (len / 2));
